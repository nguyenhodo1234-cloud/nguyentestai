  ...
  return (
    ...
    >
      ...
      <div className="space-y-0">
        {activities.map((a, i) => (
          <div key={a.id} className={`flex items-start gap-3 py-3 ${i !== activities.length - 1 ? 'border-b border-slate-800' : ''}`}>
            <img src={a.avatar} className="w-8 h-8 rounded-lg flex-shrink-0" alt="" />
            <div className="flex-1 min-w-0">
              <p className="text-sm text-slate-300">
                <span className="font-medium text-white">{a.user}</span>{' '}
                {a.action}{' '}
                <span className="text-blue-400">{a.target}</span>
              </p>
