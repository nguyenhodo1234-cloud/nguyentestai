...
export default function StatsCard({ card, index }: Props) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl bg-slate-900 border border-slate-800 p-6 hover:border-slate-700 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 group"
    >
      {/* Gradient overlay */}
      <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl ${card.gradient} opacity-10 rounded-bl-full -mr-8 -mt-8 group-hover:opacity-20 transition-opacity`} />
